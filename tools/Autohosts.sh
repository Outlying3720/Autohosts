...
for i in `seq 3`; do
	wget --no-check-certificate -N $rhcloud -o outlog.log #下载并保存记录到outlog.log
	stage=`grep -c 'not retrieving' outlog.log` #1.16版本兼容 无304 而是not retrieving
	if [ $stage == 0 ]; then
		stage=`grep -c '200' outlog.log`
		if [ $stage == 0 ]; then
			echo "获取失败第 $i 次"
			sleep 5
		else
...
if [ $stage == 0 ]; then
	...
	for i in `seq 3`; do
		wget -N $sinapp -o outlog.log #下载并保存记录到outlog.log
		stage=`grep -c 'not retrieving' outlog.log` #1.16版本兼容 无304 而是not retrieving
		if [ $stage == 0 ]; then
			stage=`grep -c '200' outlog.log`
			if [ $stage == 0 ]; then
...
if [ $stage == 0 ]; then
	...
	echo '尝试从Github中获取'
	for i in `seq 3`; do
		wget --no-check-certificate $github -o outlog.log -O hosts_new
		if [[ -s hosts_new ]]; then
			echo '从Github中获取成功'
